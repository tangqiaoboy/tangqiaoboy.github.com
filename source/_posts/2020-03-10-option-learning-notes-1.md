...
与虚值期权相返，实值期权随着时间的流逝，它的 Delta 会趋近于 1。因为这个时候，期权的时间价值几乎没有了，它的价格反映的就是行权价和股价之间的差。

书中对 Delta 还有一种另类的看待角度：**Delta 可以看作期权最终到期时为实值期权的概率**。虽然数学解释不严谨，但是很有效。

### 其它指标

除了 Delta 指标之外，期权还有 Theta，Rho, Vega，Gamma 等指标，它们的含义我放到下一篇小结中展开。
